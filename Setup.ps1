...
if ($rule) {
    ...
    Say '  관리자 권한이 없어 방화벽 규칙을 추가하지 못했습니다.' 'Yellow'
...
try {
    ...
    Say '  전용 실행 파일 없이도 동작합니다. 작업 관리자에 Windows PowerShell 로 표시될 뿐입니다.' 'DarkGray'
}
...
---------------------------------------
  ...
  답장 한 통마다 문자 요금이 듭니다.
  부담이면 [카톡 릴레이] > [설정 편집]에서 smsReply를 false로 바꾸세요.
